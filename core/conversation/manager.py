...
class ConversationManager:
    ...
    def _default_local_source(self, driver, gate):
        import config
        # Duplex/DTLN tier (echo cancel for open speakers) is OPT-IN, OFF by default. If it can't
        # load — most commonly the DTLN onnx models are absent (they live in gitignored user/) — it
        # falls back to the proven headphone tier so conversation mode never breaks.
        tier = str(getattr(config, "CONVERSATION_AUDIO_TIER", "headphone")).lower()
        if tier == "duplex":
            src = None
            try:
                ...
                logger.info("[CONV] using duplex/DTLN audio tier")
                return src
                ...
                if src is not None:
                    try:
                        ...
                        pass
        # headphone tier (default): input-only mic, no DTLN, no model dependency
